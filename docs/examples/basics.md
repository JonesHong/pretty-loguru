...
# 只輸出到控制台
log_to_targets(logger, "這條訊息只在控制台顯示", level="INFO", console_only=True)
log_to_targets(logger, "✅ 控制台專屬成功訊息", level="SUCCESS", console_only=True)

# 只輸出到檔案
log_to_targets(logger, "這條訊息只會記錄到檔案", level="INFO", file_only=True)
log_to_targets(logger, "檔案專屬錯誤記錄", level="ERROR", file_only=True)
