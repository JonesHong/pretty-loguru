...
def api_error_simulation():
    ...
    for scenario in api_scenarios:
        ...
        api_error_data.append({
            "端點": scenario["endpoint"],
            "方法": scenario["method"],
            "狀態碼": str(scenario["status_code"]),
            "錯誤類型": scenario["error"],
            "響應時間": f"{response_time:.1f}ms"
        })
...
def retry_mechanism_demo():
    ...
    for scenario in retry_scenarios:
        ...
        retry_results.append({
            "場景": scenario["name"],
            "最大重試": str(scenario["max_retries"]),
            "成功率": f"{scenario['success_rate']*100:.0f}%",
...
def error_analysis_and_reporting():
    ...
    for category, percentage in error_stats["error_distribution"].items():
        distribution_data.append({"類別": category, "百分比": f"{percentage}%"})
