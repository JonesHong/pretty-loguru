...
logger = create_logger("simple_api", log_path="./logs")
...
async def get_user(user_id: str):
    ...
    logger.file_info(f"查詢用戶 - ID: {user_id}, IP: client_ip")
    
    if user_id not in users_db:
        logger.warning(f"用戶不存在：{user_id}")
        logger.file_warning(f"404 錯誤 - 用戶 {user_id} 不存在")
        raise HTTPException(status_code=404, detail="用戶不存在")
    
    user = users_db[user_id]
    logger.success(f"成功返回用戶資訊：{user['name']}")
    logger.file_success(f"成功查詢用戶 {user_id} - {user['name']}")
...
async def create_user(user_data: dict):
    ...
    if not name or not email:
        ...
        logger.file_error(f"創建用戶失敗 - 數據：{user_data}")
    ...
    logger.file_success(f"新用戶創建 - ID: {new_id}, Name: {name}, Email: {email}")
